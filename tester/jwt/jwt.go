...
type JwtService interface {
	CreateToken(user entity.User) (serializer.Token, error)
	ValidateToken(accessToken string) (entity.User, error)
	ValidateRefreshToken(model serializer.Token) (entity.User, error)
	createRefreshToken(token serializer.Token) (serializer.Token, error)
}

type Jwt struct{}

func (j Jwt) CreateToken(user entity.User) (serializer.Token, error) {
	var err error
...
func (Jwt) ValidateRefreshToken(model serializer.Token) (entity.User, error) {
	sha1 := sha1.New()
...
func (Jwt) createRefreshToken(token serializer.Token) (serializer.Token, error) {
	sha1 := sha1.New()
	io.WriteString(sha1, os.Getenv("SECRET_KEY"))
